...
<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (Heading text says 'View Routing Associations' but it is under 'Routing Propagation' (k63); ko k64 is '라우팅 전파 목록 보기' (View Routing Propagations) — the target heading wording is a copy-paste error from the Routing Association section, but it is matched to k64 by position and meaning of the parent context. No extra entry needed — matched above.) -->
...
<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (Heading text says 'View multicast domains' but it is under 'Multicast Association' (k105); ko k106 is '멀티캐스트 연결 목록 보기' (View Multicast Associations) — wording is a copy-paste error but matched to k106 by position and parent context. No extra entry needed — matched above.) -->
